...
interface EditableMoviePlotProps {
  initialPlot: string
  movieTitle?: string
  movieMeta?: {
    id?: number
...
export function EditableMoviePlot({ initialPlot, movieTitle, movieMeta, onSave }: EditableMoviePlotProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (onSave) {
        ...
        const slug = movieMeta?.id ? `${movieTitle?.toLowerCase().replace(/\s+/g, '-')}-${movieMeta.id}` : undefined
        ...
        console.log(`Salvataggio trama per: ${movieTitle} (slug: ${slug})`)
        ...
        console.log("Trama film salvata con successo:", movieTitle)
      }
      
      setIsEditing(false)
    } catch (error: any) {
      console.error("Errore durante il salvataggio della trama del film:", error)
    ...
    try {
      ...
      const response = await fetch('/api/genera-biografia-magica', {
        ...
        body: JSON.stringify({
          ...
          releaseDate: movieMeta?.releaseDate,
          directors: movieMeta?.directors,
          cast: movieMeta?.cast,
          genres: movieMeta?.genres,
          runtime: movieMeta?.runtime,
          voteAverage: movieMeta?.voteAverage
        })
      });
    ...
    return (
      <div className="relative">
        ...
        <textarea
          ...
          placeholder={`Inserisci la trama${movieTitle ? ` di "${movieTitle}"` : ''}...`}
        />
  ...
  return (
    <div className="relative">
      ...
      >
        {plot || `Nessuna trama${movieTitle ? ` per "${movieTitle}"` : ''} disponibile. Doppio click per aggiungere.`}
      </div>
